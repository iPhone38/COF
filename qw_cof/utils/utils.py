...
from .debug import print_model_inputs, print_outputs
...
class GenerationMixin(ContinuousMixin):
    ...
    def _cache_dependant_input_preparation(
        ...
        if inputs_embeds is not None and input_ids.shape[1] == 0:  # Exception 4
            ...
            #here we just select the cache_positon induce
    ...
    def _update_model_kwargs_for_generation(
        ...
        if model_kwargs.get("use_cache", True):
            model_kwargs["cache_position"] = model_kwargs["cache_position"][-1:] + num_new_tokens #固定是增加1
        else:
    ...
    def generate(
        ...
        **kwargs,
        ...
        use_cof = kwargs.pop("use_cof", None)
        if use_cof:
            processor = kwargs.pop("processor", None)
            image_inputs_batch = kwargs.pop("image_inputs_batch", None)
        ...
        if use_cof:
            zoomin_token_id = getattr(generation_config, "zoomin_token_id", None)
            if zoomin_token_id is None:
                raise ValueError("COF is enabled, but the model does not support COF.")
        ...
        # "temperature": 1e-06,
        # "zoomin_token_id": 151665    #这个就是指令token
        # }

        
        # model_kwargs: 就是函数接口其他的传入的参数的字典
        ...
        #_prepare_model_inputs其实就是从model_kwargs解析出input_ids字段
        #inputs_tensor:就是输入token的张量
        ...
        # generation_mode=<GenerationMode.SAMPLE: 'sample'>
        ...
        if generation_mode == GenerationMode.ASSISTED_GENERATION:
            ...
            # input_ids= [bs,pad_seq_len]  [2, 776]
            # model_kwargs['image_grid_thw']: tensor([[ 1, 48, 62], [ 1, 20, 14]]
            # model_kwargs['pixel_values'].shape=torch.Size([3256, 1176])
            
            # 12. run sample (it degenerates to greedy search when `generation_config.do_sample=False`)
            if use_cof and zoomin_token_id:
                print("USE COF")
                result = self._sample_cof(
                    input_ids,
                    logits_processor=prepared_logits_processor,
                    stopping_criteria=prepared_stopping_criteria,
                    generation_config=generation_config,
                    synced_gpus=synced_gpus,
                    zoomin_token_id = zoomin_token_id,
                    processor = processor,
    ...
    def _sample_cof_0(
        ...
        generation_config: GenerationConfig,
        synced_gpus: bool,
        zoomin_token_id: int,          # <|image_zoomin|> 的 token ID
        processor: AutoProcessor,    # 用于图像裁剪和处理的处理器
        image_inputs_batch: list,      # 原始输入图像（批处理）
        streamer: Optional["BaseStreamer"],
        **model_kwargs,
    ) -> Union[GenerateNonBeamOutput, torch.LongTensor]:
        ...
        batch_size, cur_len = input_ids.shape[:2]
        this_peer_finished = False
        model_forward = self.__call__  #定义模型forward函数
        unfinished_sequences = torch.ones(batch_size, dtype=torch.long, device=input_ids.device)
        model_kwargs = self._get_initial_cache_position(cur_len, input_ids.device, model_kwargs)

        # 新增：初始化 CoF 相关参数
        model_kwargs["history"] = []  # 存储历史步骤的 (r_i, y_i, o_i)
        model_kwargs["visual_tokens"] = None  # 动态注入的视觉特征
        box_start_token_id = processor.tokenizer.convert_tokens_to_ids("<|box_start|>")
        box_end_token_id = processor.tokenizer.convert_tokens_to_ids("<|box_end|>")
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # 更新 model_kwargs（包括 past_key_values）
            model_kwargs = self._update_model_kwargs_for_generation(
                outputs,
                model_kwargs,
                is_encoder_decoder=self.config.is_encoder_decoder,
            ...
            for batch_idx in range(batch_size):
                current_token = next_tokens[batch_idx].item()
                if current_token == box_start_token_id:
                    # 解析边界框坐标（假设格式为 <|box_start|>[x1,y1,x2,y2]<|box_end|>）
                    box_coords = self._parse_box_coords(input_ids[batch_idx], processor.tokenizer)
                    if box_coords is not None:
                        # 裁剪并编码区域
                        image = image_inputs_batch[batch_idx]  # 假设 image_inputs_batch 是 PIL 图像列表
                        cropped_region = processor.image_processor.crop(image, box_coords)
                        resized_region = processor.image_processor.resize(cropped_region, (224, 224))  # 假设输入尺寸为 224x224
                        region_tensor = processor.image_processor.preprocess(resized_region, return_tensors="pt").to(input_ids.device)
                        visual_tokens = self.vision_encoder(region_tensor)  # 提取视觉特征

                        # 更新历史
                        model_kwargs["history"].append({
                            "box_coords": box_coords,
                            "visual_tokens": visual_tokens
                        })

                        # 注入 <|image_zoomin|> 标记和视觉特征
                        next_tokens[batch_idx] = zoomin_token_id
                        model_kwargs["visual_tokens"] = visual_tokens  # 将在下一步通过 prepare_inputs_for_generation 处理
    ...
    def _parse_box_coords(self, token_ids: torch.Tensor, tokenizer) -> Optional[list]:
        """解析边界框坐标（格式：<|box_start|>[x1,y1,x2,y2]<|box_end|>）"""
        tokens = tokenizer.convert_ids_to_tokens(token_ids.tolist())
        try:
            box_start_idx = tokens.index("<|box_start|>")
            box_end_idx = tokens.index("<|box_end|>", box_start_idx)
            coords_str = "".join(tokens[box_start_idx + 1 : box_end_idx]).strip("[]")
            return list(map(int, coords_str.split(",")))
        except ValueError:
            return None
    ...
    def _sample_cof(
        ...
        **model_kwargs,
        ...
        # init values
        pad_token_id = generation_config._pad_token_tensor   #pad_token_id=tensor(151643, device='cuda:0')
        output_attentions = generation_config.output_attentions  # 是否返回注意力权重  false
        output_hidden_states = generation_config.output_hidden_states # 是否返回所有隐藏层的状态 false
        output_scores = generation_config.output_scores    # 是否返回每个生成步骤的预测分数（未归一化的logits） false
        output_logits = generation_config.output_logits  # 与output_scores类似，但可能包含更原始的logits值 false
        return_dict_in_generate = generation_config.return_dict_in_generate #控制返回结果为结构化字典还是纯Tensor False
        has_eos_stopping_criteria = any(hasattr(criteria, "eos_token_id") for criteria in stopping_criteria) #检查是否设置了基于eos_token_id的终止条件 True
        do_sample = generation_config.do_sample #使用采样（如top-k/top-p） 也就是是否有概率抽样过程，True
        ...
        this_peer_finished = False
        unfinished_sequences = torch.ones(batch_size, dtype=torch.long, device=input_ids.device)
        model_kwargs = self._get_initial_cache_position(cur_len, input_ids.device, model_kwargs)#计算初始输入的kvcache的坐标，追加到model_kwargs字典中
        model_forward = self.__call__  #定义模型forward函数
        compile_forward = self._valid_auto_compile_criteria(model_kwargs, generation_config)
        if compile_forward:  #编译加速 false
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device): 
            ...
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs) #具体模型派生的输入处理函数
            
            #print
            if USE_DEBUG:
                print_model_inputs(model_inputs)
            #cache_position, past_key_values, input_ids, inputs_embeds, position_ids, attention_mask, pixel_values , pixel_values_videos , image_grid_thw, second_per_grid_ts, use_cache
            
            # prepare variable output controls (note: some models won't accept all output controls)
            model_inputs.update({"output_attentions": output_attentions} if output_attentions else {}) #追加输出格式，是否输出attention
            model_inputs.update({"output_hidden_states": output_hidden_states} if output_hidden_states else {}) #追加输出格式，是否输出中间结果
            
            if is_prefill:    
                outputs = self(**model_inputs, return_dict=True)
                is_prefill = False
            else:
                if append_visul_token_num != 0:
                    model_inputs["pixel_values"] = model_kwargs["pixel_values"][-append_visul_token_num:]
                # outputs = model_forward(**model_inputs, return_dict=True)
                outputs = model_forward(**model_inputs, return_dict=True)
            ...
            #检测是否出现指令token zoomin,如果出现,则会追加新的视觉上下文，也就是需要更新input_ids 和 model_kwargs
            zoomin_exist, apend_inputs = CoF.process_next_tokens(next_tokens)
            if zoomin_exist:
                model_kwargs, input_ids, append_visul_token_num = CoF.append_visual_tokens(apend_inputs, model_kwargs, input_ids)
            append_visul_token_num = 0
            # CoF.print_state()
